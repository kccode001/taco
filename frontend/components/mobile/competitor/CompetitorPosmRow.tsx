...
export type CompetitorPosmKondisi = "baik" | "perlu_ganti" | "hilang";

export interface CompetitorPosmEntry {
  id: string;
  nama: string;
  kondisi: CompetitorPosmKondisi | null;
  photo_url: string | null;
}

const KONDISI: { key: CompetitorPosmKondisi; label: string; cls: string }[] = [
  {
    key: "baik",
    label: "Baik",
...
interface CompetitorPosmRowProps {
  entry: CompetitorPosmEntry;
  onChange: (entry: CompetitorPosmEntry) => void;
  onRemove: () => void;
  onUploadPhoto: (file: File) => Promise<string | null>;
}

export function CompetitorPosmRow({
  entry,
  onChange,
  onRemove,
  onUploadPhoto,
}: CompetitorPosmRowProps) {
    ...
    <div className="border-b border-taco-divider py-3 last:border-b-0">
      <div className="flex items-start gap-2 mb-2">
        <input
          ...
          placeholder="Nama aset POSM kompetitor (mis. Spanduk, Banner, Display…)"
        ...
        <button
          ...
          className="w-11 h-11 flex items-center justify-center rounded-[8px] border-[1.5px] border-taco-border text-taco-sub"
        >
