...
function FallbackInvoice(id: string): TaroInvoiceDetail {
  ...
  // Fall back to the first cached detail so the page never renders empty.
...
export default function TaroInvoiceDetailPage() {
  ...
  if (!invoice) {
    return (
      <div className="text-[13px] text-taco-muted">Memuat invoice…</div>
    );
  }
...
function EditLineItemModal({
  ...
  // Load SKU list once. The taco-skus endpoint accepts ?search= but we
  // filter client-side for snappy keystroke feedback in the dropdown.
  useEffect(() => {
    (async () => {
      try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        // Swallow BE errors — local mock update keeps the UX flowing
        // until /api/taro-invoices/line-items is live.
      }
      onSaved({
        ...line,
        matched_sku_id: selectedSku.id,
